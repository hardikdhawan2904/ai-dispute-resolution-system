...
const CASE_STATUSES: CaseStatus[] = [
  "Dispute Raised","Under Investigation","Pending Documents","Escalated","Resolved","Rejected","Closed",
];
...
export default function OpsCaseDetail() {
  ...
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    Promise.all([getCase(id), getAuditLogs(id), getWorkflowStates(id)])
      .then(([c, a, w]) => {
        setCaseData(c);
        setAuditLogs(a.audit_logs);
        setWorkflowStates(w.workflow_states);
      })
      .catch(() => { toast.error("Case not found"); router.push("/ops/disputes"); })
    ...
    <>
      <div className="flex items-center gap-2 mb-6 text-sm">
        <Link href="/ops/disputes" className="text-bfsi-text-dim hover:text-bfsi-text transition-colors flex items-center gap-1">
          <ArrowLeft className="w-4 h-4" /> Case Management
        </Link>
      ...
      <div className="bfsi-card p-4 mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <select className="bfsi-select text-sm py-1.5 pr-8 w-auto" value={caseData.status}
            onChange={(e) => handleStatusUpdate(e.target.value)} disabled={updatingStatus}>
              ...
              <div className="space-y-2">
                {workflowStates.map((ws) => (
                  <div key={ws.id} className="flex items-center justify-between p-3 bg-bfsi-muted rounded-lg">
                    <div className="flex items-center gap-2">
                      {ws.success ? <CheckCircle className="w-4 h-4 text-green-400" /> : <AlertTriangle className="w-4 h-4 text-red-400" />}
                      <span className="text-xs font-mono text-bfsi-text">{ws.node_name}</span>
                    </div>
                    {ws.execution_time_ms && (
                      <span className="text-[10px] text-bfsi-text-dim font-mono">{ws.execution_time_ms.toFixed(0)}ms</span>
                    )}
                  </div>
                ))}
              </div>
